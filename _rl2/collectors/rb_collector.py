...
from _rl2.collectors.collector import GeneralCollector
import _rl2
...
class RBCollector(GeneralCollector):
    ...
    def reset_buffer(self):
        if self.per:
            self.buffer = getattr(_rl2.modules, 'PrioritizedReplayBuffer')(
                self.args.rb_size,
                s_shape=self.env.observation_space.shape
            )
        else:
            self.buffer = getattr(_rl2.modules, 'ReplayBuffer')(
                self.args.rb_size,
                s_shape=self.env.observation_space.shape
            )
    ...
    def collect(self, info=None, **kwargs):
        self.eps = max(1.0 - self.frames / (self.args.steps * 0.02), 0.0) * 0.99 + 0.01
        obs = self.obs.copy()
        if len(obs.shape) == 4:
