...
from _rl2.settings import PROJECT_ROOT, LOG_LEVELS, LOG_DIR
...
class Logger:
    def __init__(self, name, args=None, log_dir=None):
        self.args = args
        if log_dir is None:
            self.log_dir = os.path.join(PROJECT_ROOT, LOG_DIR, self.args.tag, datetime.now().strftime("%Y%m%d%H%M%S"))
