...
class MADDPGModel(DDPGModel):
    def __init__(
        ...
        self.q, self.optim_cr, self.q_trg = self._make_mlp_optim_target(
            critic, obs_dim + joint_action_shape[0], 1, optim_cr
        )
...
class MADDPGAgent(MAgent):
        ...
        for model, obs in zip(self.models, obss):
            ...
            if model.discrete:
                if np.random.random() < self.eps:
                    action = [
                        np.random.randint(model.action_shape[0])
                        for _ in action
                    ]
    ...
    def step(self, states, actions, rewards, dones, next_states):
        if self.models[0].discrete:
            ...
            for i, action in enumerate(actions):
                action_onehots.append(
                    np.eye(self.models[i].action_shape[0])[action]
                )
            actions = action_onehots
        self.collect(states, actions, rewards, dones, next_states)
        self.episode_score += sum(rewards)
        if all(dones):
            self.mean_rew.append(self.episode_score)
            self.episode_score = 0.0

        start_train = self.curr_step > self.init_collect
        if self.train_at(start_train):
            for _ in range(self.config.num_epochs):
                self.train()
        if self.update_at(start_train):
            for model in self.models:
                model.update_target()
        if self.log_at(start_train):
            curr_time = time.strftime(
                '[%Y-%m-%d %I:%M:%S %p]', time.localtime()
            )
            print(
                curr_time,
                self.curr_step,
                sum(list(self.mean_rew)) / len(list(self.mean_rew)),
                self.loss_ac, self.loss_cr
            )
        self.curr_step += 1

    def log_at(self, start_train):
        return self.curr_step % self.log_step == 0 and start_train

    def update_at(self, start_train):
        return self.curr_step % self.update_interval == 0 and start_train

    def train_at(self, start_train):
        return self.curr_step % self.train_interval == 0 and start_train
    ...
    def train(self):
        ...
        for i, model in enumerate(self.models):
            ...
            sample_idx = np.random.randint(
                self.buffers[i].curr_size, size=self.batch_size
            )
            transitions = [
                buffer.sample(self.batch_size, idx=sample_idx)
                for buffer in self.buffers
            ]
            # transitions.insert(i, main_transition)
            loss_cr = self.loss_func_cr(
                transitions, self.models, index=i, gamma=self.gamma
            )
            model.step_cr(loss_cr)
            self.loss_cr = loss_cr.item()
            loss_ac = self.loss_func_ac(
                transitions, self.models, index=i, gamma=self.gamma
            )
    ...
    def collect(
        ...
        for i, (obs, ac, rew, done, next_obs) in \
                enumerate(zip(obss, acs, rews, dones, next_obss)):
            self.buffers[i].push(obs, ac, rew, done, next_obs)
