...
# TENSORBOARD = 25
# logging.addLevelName(TENSORBOARD, 'TENSORBOARD')
...
class TensorboardLogger(logging.Logger):
    ...
    def __init__(self, name, level='INFO', log_dir=None):
        logging.addLevelName(self.TENSORBOARD, 'TENSORBOARD')
    ...
    #     self.log(lvl, colored(msg, color))
    ...
    #     self.log(tb, "DEBUG")
    #
    # def scalar_summary(self, info, step, lvl=None, tag='values'):
    #     assert isinstance(info, dict), "data must be a dictionary"
    #     # flush to terminal
    #     if self.log_level <= LOG_LEVELS[lvl]['lvl']:
    #         key2str = {}
